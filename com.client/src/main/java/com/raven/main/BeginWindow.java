...
class MyPlane extends JPanel implements MouseListener{
	...
	int rectwidth = 180;
	int rectheight = 60;
	public static String classPath = MyPlane.class.getResource("/").getPath();
	Font beginFont = new Font("黑体",Font.BOLD,60);
	Font gameFont = new Font("黑体",Font.BOLD,30);
	static ImageIcon bgImg ;
	//static ImageIcon bgImg = new ImageIcon(GameClient.classPath +"/source/bkImg.jpg");
	...
	public MyPlane(BeginWindow beginWindow) {
		...
		bgImg = new ImageIcon(getClass().getResource("/source/bkImg.jpg"));
	...
	private void playGame() {
		try {
			...
			if(i==0) {
				...
				if(serverIp.equals("")) {
					serverIp ="127.0.0.1";
					BeginWindow.host = "http://127.0.0.1";
				}
				BeginWindow.socket = new Socket(serverIp,6666);
				
				BeginWindow.host = "http://"+serverIp;
			}
		} catch (IOException e) {
